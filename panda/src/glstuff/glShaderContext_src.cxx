...
set_state_and_transform(const RenderState *target_rs,
  ...
  int altered = _tracker.update(target_rs, _glgsg->_target_shader, modelview_transform, camera_transform, projection_transform);
  if (altered != 0) {
...
issue_parameters(int altered) {
  ...
  if (altered & _uniform_data_deps) {
    if (altered & _tracker._matrix_cache_deps) {
      _glgsg->update_shader_matrix_cache(_shader, &_tracker._matrix_cache[0], altered);
    }
    ...
    for (const UniformBlock &block : _uniform_blocks) {
      ...
      for (const UniformBlock::Binding &binding : block._bindings) {
        binding._binding->fetch_data(_tracker, scratch + binding._offset, false);
      }
...
update_shader_vertex_arrays(ShaderContext *prev, bool force) {
  ...
  // vertex colors.
  const ColorAttrib *color_attrib = _tracker._color_attrib.p();
  ...
  if (_uniform_data_deps & ShaderStateTracker::D_vertex_data) {
    _tracker.update_vertex_data(_glgsg->_data_reader);

    issue_parameters(ShaderStateTracker::D_vertex_data);
  }
...
update_shader_texture_bindings(ShaderContext *prev) {
  ...
  if (num_image_units > 0) {
    for (int i = 0; i < num_image_units; ++i) {
      ...
      int z = -1;
      int n = 0;
      PT(Texture) tex = unit._binding->fetch_texture_image(_tracker, unit._resource_id, access, z, n);
  ...
  for (size_t i = 0; i < num_textures; ++i) {
    ...
    PT(Texture) tex = unit._binding->fetch_texture(_tracker, unit._resource_id, sampler, view);
    if (tex.is_null()) {
...
attach_shader(const ShaderModule *module, Shader::ModuleSpecConstants &consts,
  ...
  if (module->is_of_type(ShaderModuleSpirV::get_class_type())) {
    ...
    if (_glgsg->_supports_spir_v) {
      ...
      if (spv->get_num_parameters() > 0 && !locations.empty()) {
        ShaderModuleSpirV::InstructionWriter writer(stream);
        writer.assign_locations(locations);
      }
