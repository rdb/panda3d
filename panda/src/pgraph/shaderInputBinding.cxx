...
make(const InternalName *name, const ShaderType *type,
  ...
  if (pieces[0] == "p3d" && language == Shader::SL_GLSL) {
    ...
    if (pieces[1] == "ClipPlane") {
      ...
      return make_data(ShaderStateTracker::D_clip_planes | ShaderStateTracker::D_view_transform,
                       [=](const ShaderStateTracker &state, void *into) {
        ...
        if (state._state_rs->get_attrib(cpa)) {
          SceneSetup *scene_setup = state._gsg->get_scene();
    ...
    if (pieces[1] == "TexAlphaOnly") {
      ...
      return make_data(ShaderStateTracker::D_texture | ShaderStateTracker::D_frame,
                       [=](const ShaderStateTracker &state, void *into) {

        const TextureAttrib *ta;

        uint32_t num_stages = 0;
        if (state._state_rs->get_attrib(ta)) {
  ...
  if (pieces[0] == "osg" && language == Shader::SL_GLSL) {
    ...
    if (pieces[1] == "ViewMatrix") {
      ...
      if (type == ShaderType::float_type) {
        return make_data(ShaderStateTracker::D_frame,
                         [](const ShaderStateTracker &state, void *into) {
  ...
  if (pieces[0].size() >= 5 &&
    ...
    else if (language == Shader::SL_Cg && matrix == "shadow") {
  ...
  if (language == Shader::SL_Cg) {
    ...
    if (pieces[0] == "attr") {
      ...
      if (pieces[1] == "material") {
        ...
        return make_data(ShaderStateTracker::D_material | ShaderStateTracker::D_frame,
                         [=](const ShaderStateTracker &state, void *into) {
          ...
          if (state._state_rs->get_attrib(target_material) && !target_material->is_off()) {
        ...
        return make_data(ShaderStateTracker::D_fog | ShaderStateTracker::D_frame,
                         [](const ShaderStateTracker &state, void *into) {

          LVecBase4f &params = *(LVecBase4f *)into;

          const FogAttrib *target_fog;
          if (state._state_rs->get_attrib(target_fog) && target_fog->get_fog() != nullptr) {
        ...
        return make_data(ShaderStateTracker::D_light | ShaderStateTracker::D_frame,
                         [=](const ShaderStateTracker &state, void *into) {

          // We don't count ambient lights, which would be pretty silly to handle
          // via this mechanism.
          const LightAttrib *target_light;
          if (state._state_rs->get_attrib(target_light) &&
    ...
    if (pieces[0] == "alight") {
      ...
      return make_data(ShaderStateTracker::D_shader_inputs | ShaderStateTracker::D_frame,
                       [=](const ShaderStateTracker &state, void *into) {
        const NodePath &np = state._shader_attrib->get_shader_input_nodepath(input);
        nassertv(!np.is_empty());
    ...
    if (pieces[0] == "texscale") {
      ...
      return make_data(ShaderStateTracker::D_texture | ShaderStateTracker::D_tex_matrix,
                       [=](const ShaderStateTracker &state, void *into) {

        const TextureAttrib *ta;
        const TexMatrixAttrib *tma;
        if (state._state_rs->get_attrib(ta) && state._state_rs->get_attrib(tma) && stage < ta->get_num_on_stages()) {
    ...
    if (pieces[0] == "texcolor") {
      ...
      return make_data(ShaderStateTracker::D_texture | ShaderStateTracker::D_frame,
        ...
        const TextureAttrib *ta;
        if (state._state_rs->get_attrib(ta) && stage < ta->get_num_on_stages()) {
    ...
    if (pieces[0] == "texconst") {
      ...
      // a state change
      int stage = atoi(pieces[1].c_str());
      return make_data(ShaderStateTracker::D_texture | ShaderStateTracker::D_tex_gen | ShaderStateTracker::D_frame,
                       [=](const ShaderStateTracker &state, void *into) {

        const TextureAttrib *ta;
        const TexGenAttrib *tga;
        if (state._state_rs->get_attrib(ta) && state._state_rs->get_attrib(tga) && stage < ta->get_num_on_stages()) {
    ...
    if (pieces[0] == "plane") {
      ...
      // state change
      CPT(InternalName) input = InternalName::make(pieces[1]);
      return make_data(ShaderStateTracker::D_frame | ShaderStateTracker::D_shader_inputs,
                       [=](const ShaderStateTracker &state, void *into) {
        const NodePath &np = state._shader_attrib->get_shader_input_nodepath(name);
        nassertv(!np.is_empty());
        const PlaneNode *plane_node;
    ...
    if (pieces[0] == "clipplane") {
      ...
      return make_data(ShaderStateTracker::D_clip_planes | ShaderStateTracker::D_frame,
                       [=](const ShaderStateTracker &state, void *into) {
        const ClipPlaneAttrib *cpa;
        state._state_rs->get_attrib_def(cpa);
    ...
    if (pieces[0] == "sys") {
      ...
      if (pieces[1] == "pixelsize" || pieces[1] == "windowsize") {
        ...
        return make_data(ShaderStateTracker::D_scene,
                         [=](const ShaderStateTracker &state, void *into) {
          *(LVecBase2f *)into = LCAST(float, state._pixel_size);
        });
      }
      if (pieces[1] == "time") {
        if (!expect_float_vector(name, type, 1, 1, true)) {
    ...
    if (pieces[0] == "shadow") {
      ...
      return make_texture(ShaderStateTracker::D_frame | ShaderStateTracker::D_light,
                          [=](const ShaderStateTracker &state, SamplerState &sampler, int &view) {

        const LightAttrib *target_light;
        state._state_rs->get_attrib_def(target_light);

        PT(Texture) tex;

        size_t num_lights = target_light->get_num_non_ambient_lights();
        if (index >= 0 && (size_t)index < num_lights) {
          tex = state._gsg->get_shadow_map(target_light->get_on_light(index));
        } else {
          // There is no such light assigned.  Bind a dummy shadow map.
          tex = state._gsg->get_dummy_shadow_map(is_cube);
        }
        if (tex != nullptr) {
    ...
    if (pieces[0] == "texpad") {
      ...
      return make_data(ShaderStateTracker::D_frame | ShaderStateTracker::D_shader_inputs,
                       [=](const ShaderStateTracker &state, void *into) {
        Texture *tex = state._shader_attrib->get_shader_input_texture(input);
...
make_shader_input(const ShaderType *type, CPT_InternalName name) {
  if (const ShaderType::SampledImage *sampler = type->as_sampled_image()) {
    ...
    int dep = ShaderStateTracker::D_shader_inputs | ShaderStateTracker::D_frame;
    if (matrix->get_num_columns() == 4) {
      uint32_t num_rows = matrix->get_num_rows();
      if (num_rows == 4) {
        if (matrix->get_scalar_type() == ShaderType::ST_double) {
          return make_data(dep, [=](const ShaderStateTracker &state, void *into) {
            state._shader_attrib->get_shader_input_matrix(name, *(LMatrix4d *)into);
          });
        } else {
          return make_data(dep, [=](const ShaderStateTracker &state, void *into) {
            state._shader_attrib->get_shader_input_matrix(name, *(LMatrix4f *)into);
          });
        }
      } else {
        if (matrix->get_scalar_type() == ShaderType::ST_double) {
          return make_data(dep, [=](const ShaderStateTracker &state, void *into) {
            LMatrix4d tmp;
            state._shader_attrib->get_shader_input_matrix(name, tmp);
...
make_matrix(const ShaderType *type, Shader::StateMatrix input,
            CPT_InternalName arg, bool transpose, size_t offset) {
  ShaderType::ScalarType scalar_type;
  uint32_t num_elements;
  uint32_t num_rows;
  uint32_t num_cols;
...
make_matrix_compose(const ShaderType *type, Shader::StateMatrix input0,
                    ...
                    CPT_InternalName arg1, bool transpose, size_t offset) {
...
make_transform_table(const ShaderType *type, bool transpose) {
  ...
  if (num_rows == 4) {
    return make_data(ShaderStateTracker::D_vertex_data,
                     [=](const ShaderStateTracker &state, void *into) {

      const TransformTable *table = state._data_reader->get_transform_table();
...
make_slider_table(const ShaderType *type) {
  ...
  return make_data(ShaderStateTracker::D_vertex_data,
    ...
    const SliderTable *table = state._data_reader->get_slider_table();
...
make_color(const ShaderType *type) {
  return make_data(ShaderStateTracker::D_color,
                   [](const ShaderStateTracker &state, void *into) {

    if (state._color_attrib->get_color_type() == ColorAttrib::T_flat) {
      *(LVecBase4f *)into = LCAST(float, state._color_attrib->get_color());
    } else {
...
make_color_scale(const ShaderType *type) {
  return make_data(ShaderStateTracker::D_colorscale,
                   [](const ShaderStateTracker &state, void *into) {

    const ColorScaleAttrib *target_color_scale = (const ColorScaleAttrib *)
      state._state_rs->get_attrib_def(ColorScaleAttrib::get_class_slot());
...
make_texture_stage(const ShaderType *type, size_t index) {
  ...
  return make_texture(ShaderStateTracker::D_texture,
                      [=](const ShaderStateTracker &state, SamplerState &sampler, int &view) {

    // We get the TextureAttrib directly from the _target_rs, not the
    // filtered TextureAttrib in _target_texture.
    const TextureAttrib *texattrib;
    state._state_rs->get_attrib_def(texattrib);
...
make_texture_matrix(const ShaderType *type, size_t index, bool inverse, bool transpose) {
  ...
  return make_data(ShaderStateTracker::D_tex_matrix,
                   [=](const ShaderStateTracker &state, void *into) {
    ...
    uint32_t num_stages = 0;
    if (state._state_rs->get_attrib(ta) && state._state_rs->get_attrib(tma)) {
      num_stages = std::min(num_elements, (uint32_t)ta->get_num_on_stages());
    }
...
make_light_ambient(const ShaderType *type) {
  return make_data(ShaderStateTracker::D_frame | ShaderStateTracker::D_light,
                   [](const ShaderStateTracker &state, void *into) {
    const LightAttrib *target_light;
    if (state._state_rs->get_attrib(target_light) && target_light->has_any_on_light()) {
...
get_state_dep() const {
  ...
  return ShaderStateTracker::D_frame;
}
 ...
 */
void ShaderInputBinding::
fetch_data(const ShaderStateTracker &state, void *into, bool pad_rows) const {
}
...
fetch_texture(const ShaderStateTracker &state, ResourceId resource_id, SamplerState &sampler, int &view) const {
  return nullptr;
}
 ...
 */
PT(Texture) ShaderInputBinding::
fetch_texture_image(const ShaderStateTracker &state, ResourceId resource_id, ShaderType::Access &access, int &z, int &n) const {
  return nullptr;
}
...
get_state_dep() const {
  return ShaderStateTracker::get_matrix_dep(_input);
}
...
setup(Shader *shader) {
  _cache_index = shader->add_matrix_cache_item(_input, _arg, ShaderStateTracker::get_matrix_dep(_input));
}
...
void ShaderMatrixBinding::
fetch_data(const ShaderStateTracker &state, void *into, bool pad_rows) const {
  LMatrix4f m = LCAST(float, state._matrix_cache[_cache_index]);
  if (_transpose) {
...
get_state_dep() const {
  return (ShaderStateTracker::get_matrix_dep(_input0)
        | ShaderStateTracker::get_matrix_dep(_input1));
}
...
setup(Shader *shader) {
  _cache_index0 = shader->add_matrix_cache_item(_input0, _arg0, ShaderStateTracker::get_matrix_dep(_input0));
  _cache_index1 = shader->add_matrix_cache_item(_input1, _arg1, ShaderStateTracker::get_matrix_dep(_input1));
}
...
fetch_data(const ShaderStateTracker &state, void *into, bool pad_rows) const {
  LMatrix4f m;
  m.multiply(LCAST(float, state._matrix_cache[_cache_index0]),
             LCAST(float, state._matrix_cache[_cache_index1]));
  if (_transpose) {
    m.transpose_in_place();
  }
  if (pad_rows || _num_cols == 4) {
...
get_state_dep() const {
  return (ShaderStateTracker::D_scene
        | ShaderStateTracker::D_render_mode
        | ShaderStateTracker::D_transform
        | ShaderStateTracker::get_matrix_dep(Shader::SM_point_attenuation));
}
...
fetch_data(const ShaderStateTracker &state, void *into, bool pad_rows) const {
  ...
  state._state_rs->get_attrib_def(target_render_mode);

  PN_stdfloat thickness = target_render_mode->get_thickness();
  if (target_render_mode->get_perspective()) {
    LMatrix4 m = state._matrix_cache[_cache_index];
    PN_stdfloat scale = state._modelview_transform->get_scale()[1];
...
get_state_dep() const {
  return ShaderStateTracker::D_light | ShaderStateTracker::D_frame |
         ShaderStateTracker::get_matrix_dep(Shader::SM_world_to_view);
}
...
setup(Shader *shader) {
  _world_mat_cache_index = shader->add_matrix_cache_item(Shader::SM_world_to_view, nullptr, ShaderStateTracker::get_matrix_dep(Shader::SM_world_to_view));
}
...
fetch_data(const ShaderStateTracker &state, void *into, bool pad_rows) const {
  ...
  if (state._state_rs->get_attrib(target_light) &&
    ...
    LMatrix4 mat = np.get_net_transform()->get_mat() *
      state._matrix_cache[_world_mat_cache_index];
...
get_state_dep() const {
  return ShaderStateTracker::D_shader_inputs |
         ShaderStateTracker::D_view_transform |
         ShaderStateTracker::D_frame |
         ShaderStateTracker::get_matrix_dep(_matrix);
}
...
setup(Shader *shader) {
  _mat_cache_index = shader->add_matrix_cache_item(_matrix, _arg, ShaderStateTracker::get_matrix_dep(_matrix));
}
...
fetch_data(const ShaderStateTracker &state, void *into, bool pad_rows) const {
  ...
  const NodePath &np = state._shader_attrib->get_shader_input_nodepath(_input);
  nassertv(!np.is_empty());
  DirectionalLight *lt;
  DCAST_INTO_V(lt, np.node());
  LColor const &c = lt->get_color();
  LColor const &s = lt->get_specular_color();
  const LMatrix4 &cached_mat = state._matrix_cache[_mat_cache_index];
  LMatrix4 t = np.get_net_transform()->get_mat() *
               state._gsg->get_scene()->get_world_transform()->get_mat();
...
fetch_data(const ShaderStateTracker &state, void *into, bool pad_rows) const {
  ...
  p = state._matrix_cache[_mat_cache_index].xform_point(p);
...
get_state_dep() const {
  int dep = ShaderStateTracker::D_frame |
            ShaderStateTracker::get_matrix_dep(Shader::SM_world_to_view) |
            ShaderStateTracker::get_matrix_dep(Shader::SM_apiview_to_world);
  if (_input != nullptr) {
    return ShaderStateTracker::D_shader_inputs | dep;
  } else {
    return ShaderStateTracker::D_light | dep;
  }
}
...
setup(Shader *shader) {
  _world_to_view_mat_cache_index = shader->add_matrix_cache_item(Shader::SM_world_to_view, nullptr, ShaderStateTracker::get_matrix_dep(Shader::SM_world_to_view));
  _apiview_to_world_mat_cache_index = shader->add_matrix_cache_item(Shader::SM_apiview_to_world, nullptr, ShaderStateTracker::get_matrix_dep(Shader::SM_apiview_to_world));
}
 ...
 */
void ShaderLightStructBinding::
fetch_light(const ShaderStateTracker &state, const NodePath &np, void *into) const {
  ...
  if (node != nullptr) {
    ...
    if (node->is_ambient_light()) {
      ...
      LMatrix4 mat = net_transform->get_mat() *
        state._matrix_cache[_world_to_view_mat_cache_index];
      ...
      if (_shadow_view_matrix_offset >= 0) {
        ...
        LMatrix4 t = state._matrix_cache[_apiview_to_world_mat_cache_index] *
          inv_net_transform->get_mat() *
          LMatrix4::convert_mat(state._gsg->get_coordinate_system(), lens->get_coordinate_system());
...
fetch_texture(const ShaderStateTracker &state, ResourceId resource_id, SamplerState &sampler, int &view) const {
  ...
  if (_input != nullptr) {
    if (state._shader_attrib->has_shader_input(_input)) {
      light = state._shader_attrib->get_shader_input_nodepath(_input);
    } else {
      // Maybe it's a data struct after all, even though it looked like a light
      // struct initially.
      return state._shader_attrib->get_shader_input_texture(((InternalName *)_input.p())->append("shadowMap"), &sampler);
    }
  } else {
    const LightAttrib *target_light;
    size_t index = (size_t)resource_id;
    if (state._state_rs->get_attrib(target_light) && index < target_light->get_num_non_ambient_lights()) {
  ...
  PT(Texture) tex;
  if (!light.is_empty()) {
    tex = state._gsg->get_shadow_map(light);
  } else {
    // There is no such light assigned.  Bind a dummy shadow map.
    tex = state._gsg->get_dummy_shadow_map(_cube_shadow_map);
  }
  if (tex != nullptr) {
    sampler = tex->get_default_sampler();
  }
...
get_state_dep() const {
  return ShaderStateTracker::D_texture;
}
...
fetch_texture(const ShaderStateTracker &state, ResourceId resource_id, SamplerState &sampler, int &view) const {
  ...
  // filtered TextureAttrib in _target_texture.
  const TextureAttrib *texattrib;
  if (state._state_rs->get_attrib(texattrib) && index < texattrib->get_num_on_stages()) {
    int si = 0;
...
get_state_dep() const {
  return ShaderStateTracker::D_frame | ShaderStateTracker::D_shader_inputs;
}
...
fetch_texture(const ShaderStateTracker &state, ResourceId resource_id, SamplerState &sampler, int &view) const {
  ...
  PT(Texture) tex = state._shader_attrib->get_shader_input_texture(name, &sampler);
#ifndef NDEBUG
...
fetch_texture_image(const ShaderStateTracker &state, ResourceId resource_id, ShaderType::Access &access, int &z, int &n) const {
  const InternalName *name = (const InternalName *)resource_id;
  PT(Texture) tex = state._shader_attrib->get_shader_input_texture_image(name, access, z, n);
#ifndef NDEBUG
  if (!_shown_error && tex->get_texture_type() != _desired_type) {
    _shown_error = true;
    shader_cat.error()
...
fetch_data(const ShaderStateTracker &state, void *into, bool pad_rows) const {
  ...
  if (!state._shader_attrib->get_shader_input_ptr(_input, ptr_data)) {
    return;
  }
...
fetch_texture(const ShaderStateTracker &state, ResourceId resource_id, SamplerState &sampler, int &view) const {
  const InternalName *name = (const InternalName *)resource_id;
  return state._shader_attrib->get_shader_input_texture(name, &sampler);
}
...
fetch_texture_image(const ShaderStateTracker &state, ResourceId resource_id, ShaderType::Access &access, int &z, int &n) const {
  const InternalName *name = (const InternalName *)resource_id;
  return state._shader_attrib->get_shader_input_texture_image(name, access, z, n);
}
