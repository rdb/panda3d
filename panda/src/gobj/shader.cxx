...
link() {
  ...
  for (const Parameter *param : parameters) {
    ShaderInputBinding *binding = ShaderInputBinding::make(param->_name, param->_type, _language);
...
add_parameter(const InternalName *name, const ::ShaderType *type, int location) {
  ...
  param._name = name;
  param._type = type;
  param._binding = ShaderInputBinding::make(name, type, _language);
...
make_compute(ShaderLanguage lang, string body) {
  if (lang != SL_GLSL) {
    // Special exception for unit test suite
    if (lang != SL_Cg || body.find("//Cg profile hlslc") == string::npos) {
      shader_cat.error()
        << "Only GLSL compute shaders are currently supported.\n";
      return nullptr;
    }
